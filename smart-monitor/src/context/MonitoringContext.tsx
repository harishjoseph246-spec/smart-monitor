...
export function MonitoringProvider({ children }: { children: React.ReactNode }) {
  ...
  const startFullDemo = useCallback(
    (navigate: (path: string) => void) => {
      ...
      setDemoLabel('Stage 1 · Normal monitoring');
      navigate('/dashboard');
      resetToNormal(eqId);

      at(6000, () => {
        setDemoLabel('Stage 2-3 · Sensor values rising, warning detected');
        simulateWarning(eqId);
      });

      at(18000, () => {
        setDemoLabel('Stage 4-6 · Critical fault, alert & anomaly detected');
        simulateCritical(eqId);
      });

      at(30000, () => {
        setDemoLabel('Stage 7 · Smart inspection opening');
        setFocusEquipmentId(eqId);
        navigate('/inspection');
      });

      at(38000, () => {
        setDemoLabel('Stage 8-9 · Inspection failed, corrective action recommended');
        submitInspection({
          equipmentId: eqId,
          inspector: 'Auto-Demo Inspector',
          findings: 'Automated hackathon demo run — thresholds exceeded on temperature and vibration.',
          comments: 'Generated by START FULL HACKATHON DEMO.',
          correctiveAction: 'Shut down motor, inspect bearings and cooling system, verify load balance.',
        });
      });

      at(48000, () => {
        setDemoLabel('Stage 10 · Generating inspection report');
        setReportEquipmentId(eqId);
        navigate('/reports');
      });
