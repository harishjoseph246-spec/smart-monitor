...
import {
  ...
  Play, RotateCcw, Zap, Clapperboard, Filter, ArrowRight,
} from 'lucide-react';
...
export function Dashboard() {
  const {
    equipment, alerts, activity, systemOnline,
    toggleSystem, simulateWarning, simulateCritical,
    resetToNormal, startFullDemo, demoRunning,
  } = useMonitoring();
  ...
  return (
    <div className="space-y-6">
      ...
      <div ref={metricsRef} className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          <MetricCard label="Total Equipment" value={equipment.length.toLocaleString()}
            icon={Boxes} sub={`↑ ${online} online`} onClick={() => navigate('/equipment')} />,
          <MetricCard label="Online Units" value={online.toLocaleString()}
            icon={CheckCircle} tone="good" sub={`${Math.round((online / equipment.length) * 100)}% Uptime`} />,
          <MetricCard label="Active Warnings" value={openAlerts} icon={AlertTriangle}
            tone={openAlerts > 0 ? 'warn' : 'good'} sub={`↑ ${warning} since yesterday`}
          ...
          <div key={i} className={`scroll-fade-up ${metricsVisible ? 'in-view' : ''}`}
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        {/* Equipment grid */}
        <div ref={equipRef} className={`lg:col-span-2 scroll-fade-up ${equipVisible ? 'in-view' : ''}`}>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {equipment.map((eq, i) => (
              <div key={eq.id} className={`scroll-fade-up ${equipVisible ? 'in-view' : ''}`}
        ...
        {/* Right panel */}
        <div ref={rightRef} className={`space-y-4 scroll-slide-right ${rightVisible ? 'in-view' : ''}`}>
          ...
          <div className="card p-5">
            ...
            {/* Target equipment pills */}
            <div className="mb-3">
              <div className="text-[10px] font-bold uppercase tracking-wide text-gray-400 mb-1.5">Target Equipment</div>
              <div className="flex flex-wrap gap-1.5">
                {equipment.map((eq) => {
                  ...
                  return (
                    <button key={eq.id} onClick={() => setSelectedEqId(eq.id)}
                      className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border text-[11px] font-semibold transition-all duration-150 focus-ring ${
                        isSelected ? 'border-blue-500 bg-blue-600 text-white shadow-sm'
                          : isCrit ? 'border-red-200 bg-red-50 text-red-700 hover:bg-red-100'
                          : isWarn ? 'border-amber-200 bg-amber-50 text-amber-700 hover:bg-amber-100'
            ...
            <div className="grid grid-cols-2 gap-2 mb-3">
              <button onClick={toggleSystem}
                ...
                <Play size={13} /> {systemOnline ? 'Pause' : 'Resume'}
              </button>
              <button onClick={() => simulateWarning(selectedEqId)}
                ...
                <AlertTriangle size={13} /> Simulate Warning
              </button>
              <button onClick={() => simulateCritical(selectedEqId)}
                ...
                <Zap size={13} /> Critical Fault
              </button>
              <button onClick={() => resetToNormal(selectedEqId)}
                ...
                <RotateCcw size={13} /> Reset Normal
              </button>
            </div>

            <button onClick={() => equipment.forEach((eq) => resetToNormal(eq.id))}
              ...
              <RotateCcw size={12} /> Reset ALL Equipment to Normal
            </button>

            <button disabled={demoRunning} onClick={() => startFullDemo(navigate)}
